...
2. If `--plan` is selected:
   ...
   - spawn the relevant read-only domain agents from `.codex/agents/`
...
3. If `--code` is selected:
   ...
   - always spawn these 5 fixed reviewer dimensions:
     ...
     - `testing`
     - `accessibility`
   - instruct each fixed reviewer to consult only the relevant read-only domain advisors from `.codex/agents/`
